...
*** =sct
```{r,eval=FALSE}
msg1 <- "Very good. It makes intuitive sense that the dry wells would be the least likely to be functional. Many times, the variables that have the most predictive power are also the ones that are most obvious."
msg2 <- "Not so good... Maybe have a look at the hint."
msg3 <- "Incorrect. Maybe have a look at the hint."
msg4 <- "Try again!"
